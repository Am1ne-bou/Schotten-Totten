...
public class ConsoleView {
    ...
    // Mapping couleur unifié
    private String getCouleurCode(Couleur couleur) {
        if (couleur==null) return BLANC;
        switch (couleur) {
    ...
    private String formatCartePourBorne(Carte carte) {
        if (carte==null) return GRIS+"[  ]"+RESET;
        if (carte instanceof CarteTactique) return MAGENTA+BOLD+"[T]"+RESET;
        String couleurCode=getCouleurCode(carte.getCouleur());
    ...
    public void afficherPlateau(Plateau plateau, int nombreBornes) {
        ...
        for (int ligne=0; ligne<4; ligne++) {
            ...
            for (int b=0; b<nombreBornes; b++) {
                ...
                List<Carte> cartes=borne.getCartes(1);
        ...
        for (int ligne=0; ligne<4; ligne++) {
            ...
            for (int b=0; b<nombreBornes; b++) {
                Borne borne=plateau.getBorne(b);
                List<Carte> cartes=borne.getCartes(2);
                System.out.print((ligne<cartes.size()) ? " "+formatCartePourBorne(cartes.get(ligne))+" " : GRIS+" [  ] "+RESET);
            }
            System.out.println();
        }
    ...
    public void afficherMessage(String message) {
        System.out.println(BLANC+"  [i] "+message+RESET);
    }
    ...
    private String getNomCombinaison(int type) {
        switch(type) {
            case 4: return JAUNE+BOLD+"Suite Couleur!"+RESET;
            case 3: return MAGENTA+"Brelan"+RESET;
            case 2: return CYAN+"Couleur"+RESET;
            case 1: return VERT+"Suite"+RESET;
            default: return GRIS+"Somme"+RESET;
        }
    }
    ...
    public int demanderTypeCarteAJouer(boolean aCartesClan, boolean peutJouerTactique) {
        ...
        System.out.println(BLANC+BOLD+"  +-----------------------------------+"+RESET);
        if (aCartesClan) System.out.println(BLANC+"  |  "+VERT+"1."+BLANC+" Carte Clan                    |"+RESET);
        if (peutJouerTactique) System.out.println(BLANC+"  |  "+MAGENTA+"2."+BLANC+" Carte Tactique                |"+RESET);
    ...
    public int demanderChoixPioche(boolean clanDispo, boolean tactiqueDispo) {
        ...
        System.out.println(BLANC+BOLD+"  +-----------------------------------+"+RESET);
        if (clanDispo) System.out.println(BLANC+"  |  "+VERT+"1."+BLANC+" Carte Clan                    |"+RESET);
        if (tactiqueDispo) System.out.println(BLANC+"  |  "+MAGENTA+"2."+BLANC+" Carte Tactique                |"+RESET);
        System.out.println(BLANC+BOLD+"  +-----------------------------------+"+RESET);
        System.out.print(CYAN+"  > Votre choix : "+RESET);
